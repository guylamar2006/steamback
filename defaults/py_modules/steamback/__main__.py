...
def main():
    ...
    parser.add_argument(
        "--debug", "-d", help="Show debug log messages", action="store_true"
    )
    parser.add_argument(
        "--test", "-t", help="Run integration code test", action="store_true"
    )
    parser.add_argument(
        "--daemon",
        "-D",
        help="Run as a daemon that just looks for games to backup",
        action="store_true",
    )
    parser.add_argument(
        "--steampath",
    ...
    if plat_sys == "Windows":
        ...
        logger.warning(
            """HEY! KIND TESTER! No one has tried this tool before on Windows.
        ...
        # please send in a pull-request if you can!
        logger.warning(
            """HEY! KIND TESTER! No one has tried this tool before on the Mac.
            If it works (or not) could you send an email to the author: kevinh@geeksville.com?
            If it doesn't work and you know a little python you could probably contribute an easy fix - see our github!"""
        )
        ...
        steam_deck_dir = os.path.join(
            os.path.expanduser("~"), ".local", "share", "Steam"
        )
        steam_dir = (
            steam_deck_dir
            if os.path.exists(steam_deck_dir)
            else os.path.join(os.path.expanduser("~"), ".steam", "debian-installation")
        )  # the default case
    if args.steampath is not None:
        steam_dir = (
            args.steampath
            if os.path.isabs(args.steampath)
            else os.path.abspath(args.steampath)
        )

    if not os.path.exists(steam_dir):
        logger.error(f"Can't find steam directory at { steam_dir }, exiting...")
        return
